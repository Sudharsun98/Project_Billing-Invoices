...
function renderPieChart(container, labels, data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        // Special case: if slice is a full circle (or very close to it)
        if (sliceAngle >= 359.99) {
            // Render as a full circle instead of an arc
            ...
            pathData = `M ${centerX} ${centerY} L ${startX} ${startY} A ${radius} ${radius} 0 ${largeArc} 1 ${endX} ${endY} Z`;
        }
...
function openAdmin() { const m = document.getElementById('admin-modal'); if (m) m.style.display = 'flex'; renderAdminList(); toggleUpArrowBlur(true); }
function closeAdmin() { const m = document.getElementById('admin-modal'); if (m) m.style.display = 'none'; hideProductForm(); toggleUpArrowBlur(false); }
...
function closeInvoiceModal() { const m = document.getElementById('invoice-modal'); if (m) m.style.display = 'none'; toggleUpArrowBlur(false); }
...
function renderProductSales(sortBy = 'revenue') {
    const container = document.getElementById('product-sales-content');
    if (!container) return;
    ...
    for (const inv of (invoices || [])) {
        ...
        for (const item of inv.items) {
            ...
            productSales[key].totalPrice += (item.price || 0) * (item.qty || 0);
    ...
    const productImages = {};
    for (const p of products) {
        productImages[p.name] = p.img || '/api/placeholder/300/220';
    }
    ...
    switch (sortBy) {
        case 'revenue':
            salesArray.sort((a, b) => b.totalPrice - a.totalPrice);
            break;
        case 'quantity':
            salesArray.sort((a, b) => b.quantity - a.quantity);
            break;
        case 'price':
            salesArray.sort((a, b) => b.price - a.price);
            break;
        case 'name':
    ...
    if (salesArray.length === 0) {
        container.innerHTML = `
            <div class="empty-sales-state">
                ...
                <p>Start making sales to see statistics here</p>
            </div>
        `;
        return;
    }

    const topProduct = salesArray[0];
    const avgRevenue = totalRevenue / salesArray.length;
    ...
    let html = `
        <div class="sales-summary-grid">
            ...
            <div class="summary-stat-card">
                ...
                <div class="stat-value">${salesArray.length}</div>
            </div>
            ...
            <div class="summary-stat-card">
                ...
                <div class="stat-value" style="font-size: 16px; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">${escapeHtml(topProduct.name)}</div>
            </div>
        </div>
        ...
        <div style="display: flex; gap: 12px; align-items: center;">
            ...
            <select id="sales-sort-select" onchange="sortProductSales(this.value)">
                <option value="revenue" ${sortBy === 'revenue' ? 'selected' : ''}>Highest Revenue</option>
                <option value="quantity" ${sortBy === 'quantity' ? 'selected' : ''}>Most Sold</option>
        ...
        <div id="sales-products-container" style="display: grid; gap: 16px;">
    `;

    // Render products
    const maxRevenue = salesArray[0].totalPrice;
    salesArray.forEach((item, index) => {
        const img = productImages[item.name] || '/api/placeholder/300/220';
        const performancePercent = (item.totalPrice / maxRevenue) * 100;
        const isTrending = item.quantity > avgRevenue / item.price;
        const rankClass = index < 3 ? `rank-${index + 1}` : '';

        html += `
            <div class="product-sales-item ${rankClass}">
                ${index < 3 ? `<div class="rank-badge">#${index + 1} Top Seller</div>` : ''}
                ...
                <div class="product-sales-info">
                    <div class="product-name-section">
                        ...
                        <div style="font-size: 12px; color: var(--text-secondary);">${item.transactions} transactions</div>
                    </div>
                    ...
                    <div class="product-stats-grid">
                        ...
                        <div class="stat-box revenue">
                            <div class="stat-icon">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="#C81E3A">
                                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1.41 16.09V20h-2.67v-1.93c-1.71-.36-3.16-1.46-3.27-3.4h1.96c.1 1.05.82 1.87 2.65 1.87 1.96 0 2.4-.98 2.4-1.59 0-.83-.44-1.61-2.67-2.14-2.48-.6-4.18-1.62-4.18-3.67 0-1.72 1.39-2.84 3.11-3.21V4h2.67v1.95c1.86.45 2.79 1.86 2.85 3.39H14.3c-.05-1.11-.64-1.87-2.22-1.87-1.5 0-2.4.68-2.4 1.64 0 .84.65 1.39 2.67 1.91s4.18 1.39 4.18 3.91c-.01 1.83-1.38 2.83-3.12 3.16z"/>
                                </svg>
                            </div>
                            <div class="stat-label">Total Revenue</div>
                            <div class="stat-value">₹${item.totalPrice.toLocaleString()}</div>
                        </div>
                    </div>
                    
                    <div class="performance-bar">
                        <div class="performance-bar-fill" style="width: ${performancePercent}%"></div>
                    </div>
                </div>
            </div>
        `;
...
function sortProductSales(sortBy) {
    renderProductSales(sortBy);
}

function toggleUpArrowBlur(show) {
    const upArrow = document.getElementById('up-arrow');
    if (!upArrow) return;

    if (show) {
        upArrow.style.filter = 'blur(4px)';
        upArrow.style.opacity = '0.3';
        upArrow.style.pointerEvents = 'none';
    } else {
        upArrow.style.filter = 'none';
...
Object.assign(window, {
    ...
    showProductSalesModal, closeProductSalesModal, renderProductSales,
});
